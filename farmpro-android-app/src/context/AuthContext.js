...
const SESSION_KEY = 'farmpro_mobile_session';
...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = useCallback(async ({ email, password }) => {
    const result = await farmproApi.login({ email, password });
    const nextSession = {
      token: result.token,
      user: {
        id: result.userId,
  ...
  const signUp = useCallback(async ({ name, email, role, password }) => {
    const result = await farmproApi.register({ name, email, role, password });
    const nextSession = {
      token: result.token,
      user: {
        id: result.userId,
